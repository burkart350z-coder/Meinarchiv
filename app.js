...
function findDate(t){
  for(const p of [
    /(?:bis\s+zum|bis\s+|zahlbar\s+bis|fällig\s+am|Fälligkeit|Fälligkeitsdatum|Zahlbar\s+bis)\s*:?\s*(\d{1,2})[.\-\/](\d{1,2})[.\-\/](20\d{2})/i,
...
function isCreditCard(t){return /(kreditkarte|credit\s*card|kartenabrechnung|card\s*statement|visa|mastercard|cembra\s+money\s+bank|cembra)/i.test(t)}
function findCreditCardMinimum(t){
  if(!isCreditCard(t))return 0;
  for(const p of [
    /Mindestbetrag\s+bei\s+Teilzahlung\s*:?\s*(?:CHF|SFr\.?|Fr\.)?\s*([0-9' ]+[.,][0-9]{2})/i,
    ...
    /zu\s+zahlender\s+Mindestbetrag\s*:?\s*(?:CHF|SFr\.?|Fr\.)?\s*([0-9' ]+[.,][0-9]{2})/i,
    /Minimum\s*(?:Payment|Zahlung|Amount)\s*:?\s*(?:CHF|SFr\.?|Fr\.)?\s*([0-9' ]+[.,][0-9]{2})/i
...
function findAmount(t){
  ...
  const patterns=[
    /(?:Zu\s+bezahlender\s+Rechnungsbetrag|Rechnungsbetrag|Rechnungs\s*total|Total\s*Rechnung|Rechnungsbetrag\s*inkl\.?|Gesamtbetrag\s*inkl\.?\s*(?:MwSt\.?|Mehrwertsteuer)|zu\s+bezahlender\s+Gesamtbetrag|zu\s+bezahlender\s+Betrag|Totalbetrag|zu\s+bezahlen|Zahlbetrag|Saldo\s+zu\s+unseren\s+Gunsten)\s*(?:\([^)]*\))?\s*(?:in\s+CHF)?\s*:?\s*(?:CHF|SFr\.?|Fr\.)?\s*([0-9' ]+[.,][0-9]{2})/i,
    /(?:Total\s*(?:inkl\.?\s*(?:MwSt\.?|Mehrwertsteuer)|gross|brutto))\s*:?\s*(?:CHF|SFr\.?|Fr\.)?\s*([0-9' ]+[.,][0-9]{2})/i,
...
function findSender(t){
  ...
  if(/Cembra\s+Money\s+Bank\s+AG/i.test(t)||/Cembra/i.test(t))return "Cembra Money Bank AG";
...
function findNumber(t){
  const patterns=[
    /(?:Rechnungsnummer|Rechnung\s*(?:Nr\.?|Nummer)|Abrechnungsnummer|Abrechnung\s*(?:Nr\.?|Nummer))\s*:?\s*([A-Z0-9][A-Z0-9\-\/ ]{5,30})/i,
    /(?:Referenznummer|Referenz\s*(?:Nr\.?|Nummer))\s*:?\s*([0-9][0-9 '\-]{5,30})/i,
    /(?:Kontonummer|Konto\s*(?:Nr\.?|Nummer))\s*:?\s*([0-9][0-9 '\-]{5,30})/i,
    /(?:Abr\.-?Nr\.?|Abr\.-?Nummer)\s*:?\s*([0-9][0-9 '\-]{5,30})/i,
    /(?:Prämie,\s*Ref\.?|Ref\.)\s*([0-9]{6,30})/i
  ];
...
function findCategory(t,s){
  ...
  if(/bank|steuer|kredit|lohn|zins|kreditkarte|credit card|cembra/.test(x))return"Finanzen";
...
function extract(t){
  t=clean(t);const name=findSender(t),due=findDate(t),min=findCreditCardMinimum(t),total=findAmount(t),no=findNumber(t);
  return {name,due,amount:total,no,cat:findCategory(t,name),creditMinimum:min,creditCard:isCreditCard(t)};
}
function resetForm(){
  currentFile=null;$("file").value="";$("name").value="";$("amount").value="";$("minimum").value="";$("due").value="";$("no").value="";$("cat").value="Haushalt";$("rem").value="7";$("minimumWrap").hidden=true;$("preview").innerHTML="Noch keine Rechnung analysiert.";$("status").textContent="Die App liest bei Text-PDFs den tatsächlichen Inhalt.";
}
...
$("file").onchange=async e=>{
  ...
  try{
    ...
    $("name").value=d.name;$("amount").value=d.amount?d.amount.toFixed(2):"";$("minimum").value=d.creditMinimum?d.creditMinimum.toFixed(2):"";$("due").value=d.due;$("no").value=d.no;$("cat").value=d.cat;
    $("minimumWrap").hidden=!d.creditCard;
    const extra=d.creditCard?`<br>💳 Mindestbetrag: ${d.creditMinimum?money(d.creditMinimum):"nicht erkannt"}`:"";
    $("preview").innerHTML=`<b>${result.pages} Seite${result.pages===1?"":"n"}</b> · ${text.length} Zeichen<br>🏢 ${esc(d.name||"nicht erkannt")}<br>💰 Rechnungsbetrag: ${d.amount?money(d.amount):"nicht erkannt"}${extra}<br>📅 ${d.due?fmt(d.due):"nicht erkannt"}<br>📄 ${esc(d.no||"nicht erkannt")}<br>📁 ${d.cat}<br><span class="muted">Bitte die Felder vor dem Archivieren kontrollieren.</span>`;
...
$("save").onclick=async()=>{
  ...
  data.unshift({id,name:$("name").value||currentFile.name,amount:+(+$("amount").value).toFixed(2),minimum:+(+$("minimum").value||0).toFixed(2),due:$("due").value,no:$("no").value,cat:$("cat").value,rem:+$("rem").value,paid:false,paidDate:"",file:currentFile.name});
...
function render(){
  ...
  $("list").innerHTML=items.length?items.map(x=>{const i=data.indexOf(x),d=days(x.due),cls=x.paid?"paid":d<0?"danger":d<=7?"soon":"future",lab=x.paid?"✅ Bezahlt am "+fmt(x.paidDate):d<0?"🔴 Überfällig":d<=7?"🟠 Fällig in "+d+" Tagen":"🔵 Fällig "+fmt(x.due);return `<article class="doc"><b>${esc(x.name)}</b><div class="amount">${money(x.amount)}</div>${x.minimum?`<div class="muted">Mindestbetrag ${money(x.minimum)}</div>`:""}<div class="muted">Fällig ${fmt(x.due)} · ${esc(x.cat)}${x.no?" · Nr. "+esc(x.no):""}</div><span class="badge ${cls}">${lab}</span><div class="actions"><button class="btn outline" onclick="openPDF('${x.id}')">📄 PDF öffnen</button><button class="btn ${x.paid?'gray':'green'}" onclick="pay(${i})">${x.paid?"↩ Wieder öffnen":"✓ Als bezahlt markieren"}</button></div></article>`}).join(""):"<div class='empty'>Keine Rechnungen.</div>";
