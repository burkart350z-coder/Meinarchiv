// v35: Android-OCR stabil + POWERPAY-Diagnose. Nur POWERPAY-Betragsauswertung geändert.
(()=>{
 ...
 function fill(text,pages){const raw=String(text||''),t=raw.replace(/\s+/g,' '),pp=/(POWERPAY|MF\s*Group)/i.test(t)&&/(Monatsrechnung|Rechnung)/i.test(t);let total=0,min=0,due='',no='';
  ...
  if(pp){
    // 1.74 ist bei diesem POWERPAY-Layout die Zinsankündigung, kein Rechnungs-/Mindestbetrag.
    // Deshalb keine Werte unter CHF 10 automatisch übernehmen; lieber leer als falsch.
  ...
  const diag=pp?`<details style="margin-top:12px"><summary>🔎 OCR-Diagnose anzeigen</summary><div style="margin-top:8px;font-size:12px;white-space:pre-wrap;word-break:break-word;background:#f3f4f6;padding:10px;border-radius:8px"><b>Gefundene Geldwerte im Seitenfuss:</b> ${esc(candidates.join(' / ')||'keine')}\n\n${esc(f||'Kein separater Seitenfuss-Text erkannt.')}</div></details>`:'';
