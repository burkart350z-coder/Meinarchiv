// v37: POWERPAY-Bottom-OCR + anklickbare Betrag-Vorschläge. Sonst unverändert.
(()=>{
 const file=document.getElementById('file'),$=id=>document.getElementById(id);if(!file)return;
 const esc=s=>String(s||'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
 const moneyVal=s=>{let v=String(s||'').replace(/CHF/gi,'').replace(/'/g,'').trim();if(/^\d{1,4}\s+\d{2}$/.test(v))v=v.replace(/\s+/,'.');v=v.replace(/[^0-9,.-]/g,'');if(!v)return 0;if(v.includes(',')&&v.includes('.'))v=v.lastIndexOf(',')>v.lastIndexOf('.')?v.replace(/\./g,'').replace(',','.'):v.replace(/,/g,'');else if(v.includes(','))v=v.replace(',','.');const n=parseFloat(v);return Number.isFinite(n)?n:0};
 ...
 async function ocr(pdf){
  ...
  if(pdf.numPages>=1){
   $('status').textContent='🧠 POWERPAY-Beträge werden gezielt gelesen …';
   const p=await pdf.getPage(1),vp=p.getViewport({scale:4.5}),full=document.createElement('canvas');full.width=Math.ceil(vp.width);full.height=Math.ceil(vp.height);
   await p.render({canvasContext:full.getContext('2d'),viewport:vp}).promise;
   // v37: POWERPAY schreibt offenen Saldo + Mindestbetrag ganz unten auf Seite 1.
   // Nur dieses untere Band wird hochauflösend gelesen; dadurch bleiben 203.00 / 20.30 deutlich grösser als bei der Ganzseiten-OCR.
   const y=Math.floor(full.height*0.82),h=Math.max(1,Math.floor(full.height*0.175)),crop=document.createElement('canvas');crop.width=Math.floor(full.width*0.94);crop.height=h;
   const ctx=crop.getContext('2d');ctx.drawImage(full,0,y,crop.width,h,0,0,crop.width,h);
   const footerText=await recognizeCanvas(crop,'POWERPAY-Beträge');
 ...
 function footerAmounts(raw){const vals=[];const re=/\b\d{1,4}(?:[' ]\d{3})*(?:[.,]\d{2}|[ ]\d{2})\b/g;let m;while((m=re.exec(raw))){if(isDateToken(raw,m.index,m.index+m[0].length))continue;const n=moneyVal(m[0]);if(n>0&&n<100000)vals.push(n)}return [...new Set(vals.map(n=>n.toFixed(2)))].map(Number).sort((a,b)=>b-a)}
 function amountChoices(candidates){const vals=candidates.filter(n=>n>=10).slice(0,6);if(!vals.length)return '';return `<div style="margin-top:12px"><b>💡 Betrag-Vorschläge</b><div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:7px">${vals.map(n=>`<button type="button" data-ocr-amount="${n.toFixed(2)}" style="border:1px solid #cbd5e1;background:#fff;border-radius:999px;padding:8px 11px;font-weight:700;cursor:pointer">CHF ${n.toFixed(2)}</button>`).join('')}</div><div class="muted" style="font-size:12px;margin-top:5px">Antippen übernimmt den Wert als Rechnungsbetrag.</div></div>`}
 function bindAmountChoices(){document.querySelectorAll('[data-ocr-amount]').forEach(b=>b.addEventListener('click',()=>{$('amount').value=b.dataset.ocrAmount;$('amount').dispatchEvent(new Event('input',{bubbles:true}));b.textContent='✓ CHF '+b.dataset.ocrAmount;}))}
 function fill(text,pages){const raw=String(text||''),t=raw.replace(/\s+/g,' '),pp=/(POWERPAY|MF\s*Group)/i.test(t)&&/(Monatsrechnung|Rechnung)/i.test(t);let total=0,min=0,due='',no='';
  ...
  const choices=pp?amountChoices(candidates):'';
  const diag=pp?`<details style="margin-top:12px"><summary>🔎 OCR-Diagnose anzeigen</summary><div style="margin-top:8px;font-size:12px;white-space:pre-wrap;word-break:break-word;background:#f3f4f6;padding:10px;border-radius:8px"><b>Gefundene Geldwerte im POWERPAY-Betragsband:</b> ${esc(candidates.join(' / ')||'keine')}\n\n${esc(f||'Kein separater Betragsbereich erkannt.')}</div></details>`:'';
  $('preview').innerHTML=`<b>${pages} Seiten · OCR-Scan</b><br>🏢 ${pp?'POWERPAY / MF Group AG':'bitte kontrollieren'}<br>💰 ${total?'CHF '+total.toFixed(2):'Betrag nicht erkannt'}${min?'<br>💳 Mindestbetrag CHF '+min.toFixed(2):''}<br>📅 ${due||'nicht erkannt'}<br>📄 ${no||'nicht erkannt'}${choices}${diag}`;
  bindAmountChoices();
